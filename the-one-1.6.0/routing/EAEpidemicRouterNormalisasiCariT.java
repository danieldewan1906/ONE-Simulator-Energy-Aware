 ...
 * @author DANIEL
 */
public class EAEpidemicRouterNormalisasiCariT implements RoutingDecisionEngine, Estimasi{
    ...
    public EAEpidemicRouterNormalisasiCariT(Settings s) {
    }

    protected EAEpidemicRouterNormalisasiCariT(EAEpidemicRouterNormalisasiCariT proto) {
...
//                thressholdPerc = normPerc(otherHost);
    ...
    public RoutingDecisionEngine replicate() {
        return new routing.EAEpidemicRouterNormalisasiCariT(this);
    }
...
//    }
    ...
    public Double normKec(DTNHost h) {
        ...
        Iterator<Double> kec = kecepatan.iterator();
        while (kec.hasNext()) {
            normKec = Math.exp(-(kecepatan.get(kecepatan.size() - 1)) - ratarata / (2 * (Math.pow(var, 2))));
            return normKec;
        }
        return normKec;
    }
    ...
    public Double normPerc(DTNHost h) {
        ...
        Iterator<Double> perc = percepatan.iterator();
        while (perc.hasNext()) {
            normPerc = Math.exp(-(percepatan.get(percepatan.size() - 1)) - ratarata / (2 * (Math.pow(var, 2))));
            return normPerc;
        }
        return normPerc;
    }
